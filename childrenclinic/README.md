# ChildrenClinic :: A Spring Framework With Cedar Demonstration
...
## Run ChildrenClinic locally

ChildrenClinic is a [Spring Boot](https://spring.io/guides/gs/spring-boot) application built using Gradle.
...
You can have real-time updates by also running, on another command-line:
...
In its default configuration, ChildrenClinic uses an in-memory database (H2) which gets populated at startup with data.
...
At development time we recommend you use the test applications set up as `main()` methods in `ChildrenClinicIntegrationTests` (using the default H2 database and also adding Spring Boot Devtools).
